...
class SpotAsocConstraintDecoder():
    ...
    def get_state_valid_tokens(self, src_sentence, tgt_generated):
        """验证有效token的状态
        :param src_sentence: 源句子的token
        :param tgt_generated: 目标句子的token
        :return:
        ...
        elif state == 'start':  # 如果状态为开始,那么有效token就是类型start. <extra_id_0>
            valid_tokens = [self.type_start]

        # 如果状态是重新生成组合序列,<extra_id_0>的数量比<extra_id_1>的数量多一个,即接下来有效token,就是类型start和类型end. <extra_id_0> <extra_id_1>
        elif state == 'start_first_generation':
            valid_tokens = [self.type_start, self.type_end]

        # 如果状态是重新生成组合序列,<extra_id_0>的数量比<extra_id_1>的数量多两个
        elif state == 'generate_trigger':
            # 如果最后一个字符刚好是类型start,那么接下来有效token就是实体或者角色类型树中的第一个类型
            if tgt_generated[-1] == self.type_start:
                # Start Event Label
                return list(self.type_tree.keys())
            # 如果最后一个字符刚好是类型end,那么接下来有效token就是类型start和类型end <extra_id_0> <extra_id_1>
